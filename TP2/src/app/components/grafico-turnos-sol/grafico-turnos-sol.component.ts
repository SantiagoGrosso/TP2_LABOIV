...
@Component({
  selector: 'app-grafico-turnos-sol',
  templateUrl: './grafico-turnos-sol.component.html',
  styleUrls: ['./grafico-turnos-sol.component.css'],
  standalone: true,
  imports: [NgxEchartsModule, NgFor]
})
export class GraficoTurnosSolComponent implements OnInit {
  ...
  cargarTurnos(): void {
    this.authService.getTurnosSolicitadosPorMedico().subscribe((turnos) => {
      this.turnos = turnos;
      this.updateChart();
    });
  }
  ...
  public onPdfDownload(): void {
    html2canvas(this.chartContainer.nativeElement, { scale: 3 }).then(canvas => {
      ...
      pdf.save('grafico_turnos_solicitados.pdf');
    });
  }
}
